...
def main(event, context):
    ...
    # チャンネルが存在するか確認
    record = ddbutils.isExistContinuousChannelID(channel_id)
    ...
    if record is None:
        ...
        url = urls[0]
        # DynamoDBに登録
        ddbutils.registContinuousVideoList(
            video_list, ip_address, get_ttl_hours(3))
    else:
        # IPを確認
        ...
        # 登録者はカウントアップ
        if isPublishedUser:
            count = ddbutils.countupContinuousChannelID(channel_id)
